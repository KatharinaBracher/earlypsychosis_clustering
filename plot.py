...
newcolors2 = ['#0c2c84', '#1d91c0', '#64b09e', '#bfcf69'] #7fcdbb #edf8b1
...
def plot_PC(data, var, var_p_mean, index_significant):
    ...
    ax.text(0.1, 0.95, 'PCs', transform=ax.transAxes, verticalalignment='top', weight='bold')
    textstr = '\n'.join((
        '1-'+str(index_significant+1)+'***',
        str(index_significant+2) +'-'+str(feat)))
    props = dict(boxstyle='square', facecolor='blue', alpha=0.3)
    ax.text(0.1, 0.82, textstr, transform=ax.transAxes, fontsize=9,
...
def plot_data_3D(data_pca, patients = False, group=subj_data, pheno_col=pheno_col):
    ...
    #1,2,0
    ax.scatter(data_pca[:,0],data_pca[:,2],data_pca[:,1], c=group['phenotype_description'], cmap=pheno_col, s=15, lw=0, alpha=0.6)

    ax.set_xlabel('PC 1', labelpad=5)
    ax.set_ylabel('PC 3', labelpad=5)
    ax.set_zlabel('PC 2', labelpad=5)
...
def plot_cluster_result(df, number=3, patients_only=False):
    fig, ax = plt.subplots(1, 1, figsize=(1.2,1.2)) #1.2, 0.8
...
def plot_boxplots(data, columns, col_names, m=2,n=5, control=False):
    pal = ['#0c2c84', '#64b09e', '#bfcf69']
    order = [0, 1, 2]
    
    if control:
        pal = ['#0c2c84', '#64b09e', '#bfcf69','#3880d9']
        order = [0, 1, 2, 'c']

    fig, ax = plt.subplots(m,n, figsize=(8,6)) #8,4,   6.4/5.5,2 8,4 n*2,m*2.5
    for i,col in enumerate(columns):
        ...
        label = textwrap.fill(col_names[i], width=28,
